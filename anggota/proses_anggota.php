	...
	switch ($pros){
		case "tambah" :
			...
			$dt = mysql_fetch_array($que);
				...
				mysql_query("INSERT INTO t_anggota values('$kode_anggota','$dt[a]','$nama_anggota','$alamat_anggota','$jenis_kelamin','$pekerjaan','$tgl_masuk','$telp','$tempat_lahir','$tgl_lahir','aktif','$u_entry','$tgl_entri','$username','$password')");?>
				<script>
		...
		break;
		case "validasi_terima" :
		$kode=$_GET['kode_anggota'];
		$qvalidasi=mysql_query("UPDATE t_anggota set status='aktif' WHERE kode_anggota='$kode'");
		if($qvalidasi){ ?>
			<script>
		...
		$qvalidasi=mysql_query("UPDATE t_anggota set status='keluar' WHERE kode_anggota='$kode'");
		if($qvalidasi){ ?>
			<script>
				window.location="../index.php?pilih=1.4";
			</script>
